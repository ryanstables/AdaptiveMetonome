...
public:
    ...
    // counter functions...
    void iterateCounter(MidiBuffer&, int, Counter&);
...
private:
    ...
    interval=22050;            /*overwrite from host*/
    
    Counter offseCounter;
    int noteNumber=0;
...
public:
    ...
    ~TapGenerator();
    void setBPM(double x){bpm =x;};
    void initBPM(double x); // used to get playhead Info into the tap generator
